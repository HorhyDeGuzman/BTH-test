...
<template>
    ...
    >
        <header
            class="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-950/80"
        >
            <div class="container flex h-16 items-center justify-between">
                <div class="flex items-center gap-6">
                    <Link href="/admin/products" class="flex items-center gap-2.5">
                        <span
                            class="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-900 text-sm font-bold text-white shadow-soft dark:bg-white dark:text-slate-900"
                        >
                            B
                        </span>
                    ...
                    <nav class="hidden items-center gap-1 sm:flex">
                        <Link
                            href="/admin/products"
                            class="flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
                        >
                            <CubeIcon class="h-4 w-4 text-slate-500 dark:text-slate-400" />
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Link
                        href="/"
                        class="hidden text-xs font-medium text-slate-500 transition hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 sm:inline"
                    >
                        {{ t('admin.layout.view_public') }}
                    </Link>
                    <div
                        class="hidden items-center gap-2 rounded-full bg-slate-100 py-1 pl-1 pr-3 dark:bg-slate-800 sm:flex"
                    >
                        <span
                            class="flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-[11px] font-semibold text-white dark:bg-slate-700"
                        >
                            {{ initials(user?.name) }}
                        </span>
